...
LABEL_MAP = {0: "no_waving_hand", 1: "waving_hand"}
...
def _load_deim_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("hsc._deimv2_detector", _DEIM_MODULE_PATH)
...
def _create_hand_detector(model_path: Path, providers: Sequence[Any]):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to initialize DEIMv2 hand detector: {exc}") from exc
    return detector
...
def _save_epoch_diagnostics(
    ...
    for tick in cm_ax.get_xticklabels():
                ...
                results.append(
                    {
                        ...
                        "prob_waving_hand": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"whc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "whc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"whc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "whc_best_", 10)
...
def run_webcam_inference(
    ...
    hand_detector = _create_hand_detector(detector_path, detector_providers)
    LOGGER.info("Loaded DEIMv2 hand detector from %s using providers %s.", detector_path, detector_providers)
        ...
        with torch.no_grad():
            while True:
                ...
                try:
                    ...
                    LOGGER.error("Hand detector inference failed: %s", exc)
                    break

                hand_boxes = [box for box in boxes if getattr(box, "classid", None) == 26]
                best_box = max(hand_boxes, key=lambda box: box.score) if hand_boxes else None
                crop, crop_coords = _extract_hand_crop(
                    frame,
                    best_box,
                ...
                logits = model(tensor)
                prob_waving = torch.sigmoid(logits)[0].item()

                label = LABEL_MAP[int(prob_waving >= 0.5)]
                color = (0, 200, 0) if label == "waving_hand" else (50, 50, 255)
                cv2.putText(
                    frame,
                    f"Prob waving: {prob_waving:.2%}",
                    (12, 28),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    color,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="WHC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the WHC binary classifier.")
    ...
    webcam_parser.add_argument("--window_name", type=str, default="WHC Webcam")
    ...
    webcam_parser.add_argument(
        "--detector_model",
        type=Path,
        help="Optional path to the DEIMv2 ONNX model for hand detection. Defaults to the bundled dataset weight.",
    )
    webcam_parser.add_argument(
        "--detector_provider",
        type=str,
        ...
        help="ONNX Runtime execution provider for the hand detector (default: cpu).",
    )

    webcam_onnx_parser = subparsers.add_parser("webcam_onnx", help="Run real-time inference from a webcam using an ONNX model.")
    webcam_onnx_parser.add_argument("--model", type=Path, required=True, help="Path to the exported WHC ONNX model.")
    webcam_onnx_parser.add_argument("--camera_index", type=int, default=0, help="OpenCV camera index (default: 0).")
    webcam_onnx_parser.add_argument("--window_name", type=str, default="WHC Webcam (ONNX)")
