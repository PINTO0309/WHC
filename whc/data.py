...
class Sample:
    """Lightweight container describing a single cropped hand frame."""
...
POSITIVE_LABELS = {"waving_hand", "waving", "wave"}
NEGATIVE_LABELS = {"no_waving_hand", "not_waving_hand", "idle"}


def _to_label(row: pd.Series) -> int:
    for key in ("class_id", "classid"):
        if key in row and pd.notna(row[key]):
            label_id = int(row[key])
            if label_id not in (0, 1):
                raise ValueError(f"Unsupported {key} value: {label_id}")
            return label_id
    label_text = str(row.get("label", "")).strip().lower()
    if label_text in POSITIVE_LABELS:
        return 1
    if label_text in NEGATIVE_LABELS:
        return 0
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "waving_hand" if label == 1 else "no_waving_hand"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        total = len(subset)
        positive_ratio = (positives / total) * 100.0 if total else 0.0
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'waving_hand').",
            split,
            total,
...
class WHCDataset(Dataset):
    """Torch dataset that reads hand crops from disk or embedded bytes."""
